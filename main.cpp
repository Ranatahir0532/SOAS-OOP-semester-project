                ...
                if (f.verifyLogin(email, pass)) {
                    ...
                    do {
                        ...
                        if (choice == 1) f.viewDashboard();
                            ...
                            // NEW: Immediate Check before asking for further details
                            ...
                            for(int j = 0; j < studentCount; j++) {
                                if(students[j].getRollNo() == roll) { 
                                    f.markAttendance(students[j], Course(cCode,"",students[j].getSection()), date, stat); 
                            ...
                            for(int j = 0; j < studentCount; j++) {
                                if(students[j].getRollNo() == roll) { 
                                    f.assignGrade(students[j], Course(cCode,"",students[j].getSection()), grade); 
                                    studentFound = true; break; 
                                }
                            }
                            if(!studentFound) cout << "[!] Error: Student " << roll << " not found in system.\n";
